...
from . import Argument, Query, JoinBlock, UnionBlock, Optional, Filter, Expression
from . import TriplePattern as Triple
...
def p_uri_0(p):
    ...
    """
    try:
        prefix = p.stack[1].value[p[1]]
        complete_uri = prefix[:-1] + p[3] + prefix[-1:]
    except KeyError as e:
        raise KeyError("PREFIX '{}' not defined.".format(p[1]))
    p[0] = complete_uri
    #p[0] = p[1] + p[2] + p[3]
...
def parse(string):
    try:
        ...
        raise lex.LexError(e.message, e.args)
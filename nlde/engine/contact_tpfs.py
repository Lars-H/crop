...
def contact_single_tpf_server(server, query, queue, vars=None, binding={}, **kwargs):
    ...
    inverse = kwargs.get("inverse", False) and query.cardinality
    # Pagination settings.
    if inverse:
        init_page = int(math.ceil(query.cardinality / 100.0))
        page = init_page
        print query, init_page
    else:
        init_page = 1
        page = init_page
    next_page = True
        ...
        if response.status_code == 200:
            ...
            if not inverse:
                ...
                if "nextPage" in res:
                    ...
                    next_page = True
                page = page + 1
                #print page, next_page, response.url, total

            else:
                if "previousPage" in res:
                    next_page = True
                elif "hydra:previous" in res :
                    next_page = True
                page = page - 1
                #print page, next_page, total
                if page < 0:
                    break
        else:
            print "Could not contact TPF server {}. Status code: {}".format(server, response.status_code)

...
class Poly_Fjoin(object):
    ...
    def __init__(self, id_operator, variables, eddies, left_leaf, right_leaf, eddy=None, hj_request_cost_factor=1.0):
        ...
        self.hj_request_cost_factor = hj_request_cost_factor
        self.switched = False

    def __str__(self):
        return str("Poly Hash Join")
    ...
    def to_queue(self, res, source=None):
        ...
        if res.data == "EOF":
            #print "HJ Requests",self.id_operator ,  self.hj_requests, self.requests
            self.requests.update(res.requests)
            res.operator_stats.update({
                self.id_operator: {
                        "tuples_produced": self.produced_tuples,
                        "switched": self.switched,
                        "hj_cost_factor" : self.hj_request_cost_factor
                }
            })
            right_id = self.right_leaf.sources.keys()[0]
            self.requests[right_id] += self.hj_requests
    ...
    def execute(self, inputs, out, p_list=None):
        ...
        while True:
            ...
            try:
                ...
                if tuple1.data == "EOF":
                    ...
                    right_requests_performed = max(int(math.ceil(float(right_count)) / 100.0), 1)
                    right_requests_remaining = int(math.ceil(float(self.right_leaf.total_res - right_count)  / 100))
                    logger.debug(
                        "PHJ {}: Left Received: {}; Expected: {}; Right Requests Performed: {}; Right Request "
                        "Remaining: {}".format(
                            self.id_operator,
                            left_count,
                    ...
                    if self.hj_request_cost_factor * left_count < right_requests_remaining:
                        # Log Switching
                        self.switched = True

                        # Removable keys:
                        if False:
                            to_remove = sorted(self.left_table.keys())[:-1]
                        ...
                        # Set Hash join requests (assuming page size = 100)
                        self.hj_requests = max(int(math.ceil(float(right_count)) / 100.0), 1)
        ...
        if len(self.tuples_to_probe) > 0:
            logger.debug("PHJ {}: Switched to NLJ Mode".format(self.id_operator))
            # Create DP
            right_id = self.right_leaf.sources.keys()[0]
            self.right_operator = DependentOperator(right_id, self.right_leaf.server,

...
info_logger = logging.getLogger("nlde_logger")


class Poly_Bind_Join(object):

    def __init__(self, id_operator, joinvars, eddies, eddy=None, left_card=-1, switch_rule = 1, height=1):
        ...
        self.switch_rule_id = switch_rule
        self.switch_stats = {}
        self.nlj_requests = 0
    ...
    def to_queue(self, res, source=None):
        ...
        if res.data == "EOF":
            if self.switched:
                self.requests[self.right_operator.source_id] += self.nlj_requests
            self.requests.update(res.requests)
            res.requests.update(self.requests)
            res.operator_stats.update({
                self.id_operator: {
                    "tuples_produced" : self.produced_tuples,
                    "switched": self.switched,
                    "switch_rule" : self.switch_rule_id
                }
            })
    ...
    def switch_operation(self):
        ...
        if self.switch_rule_id == 1:
            return self.tuples_left_cnt > self.right_hash_join_requests

        elif self.switch_rule_id == 2:
            return self.tuples_left_cnt > self.right_hash_join_requests / math.sqrt(self.height)

        elif self.switch_rule_id == 3:
            return self.tuples_left_cnt > self.right_hash_join_requests * math.sqrt(self.height)
        else:
            return False

    def execute(self, inputs, out, p_list):
        ...
        self.tuples_left_cnt = 0
        pagesize = 100.0
        self.right_hash_join_requests = int(math.ceil(self.right.query.cardinality / pagesize))
        #self.pro_nlj_delta = right_hash_join_requests - self.left_est_card
        ...
        if switch_mode:
            self.switched = True
            logger.debug("NLJ {}: Switching Mode to Hash Join".format(self.id_operator))
            logger.debug("NLJ {}: cnt: {};  right_plan request "
                         "remaining: {}; Threshold: {}".
                         format(self.id_operator, self.tuples_left_cnt,
                                self.right_hash_join_requests, self.right_hash_join_requests * math.sqrt(self.height)))
            #print (tuple1)
            ...
            # TODO: Add switch stats to EOF
            self.to_queue(tuple1, "eof")
    ...
    def hash_join_mode(self):
        ...
        # Save request count
        self.nlj_requests = self.requests[self.right.source_id]

        # Create Independent Operator for the right_plan side
        self.right_operator = IndependentOperator(self.right.source_id, self.right.server, self.right.query,
                                                  self.right.sources_desc, eofs_desc=self.right.sources_desc,
                                                  inverse=False)

        self.requests[self.right_operator.source_id] = 0
        # Create Queue for it
        self.right = Queue()
    ...
    def stage1_hash_join(self, tuple1, tuple_rjttable, other_rjttable):
        ...
        if tuple1.data != "EOF":
            ...
            #
            #keys = set(tuple1.requests.keys()).union(set(self.requests.keys()))
            #for key in keys:
            #    self.requests[key] = self.requests.get(key, 0) + tuple1.requests.get(key, 0)

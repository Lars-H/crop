...
class LogicalPlan(object):
    ...
    def average_cost(self, cost_model):
        ...
        if not self.cost:
            self.cost = self.compute_cost(cost_model)
        base_cost = self.cost # (cost_model)
        base_cost_model_cardinality = self.cardinality
        ...
        # 14.02.21: Added line to reset cost
        self.compute_cost(cost_model)

        if len(nodes_cost) > 0:
            self.__average_cost = median(nodes_cost)
        else:
            self.__average_cost = base_cost

        # 14.02.21: Added line to reset cardinality
        self.cardinality = base_cost_model_cardinality

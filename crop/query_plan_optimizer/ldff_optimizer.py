...
class LDFF_Optimizer(Optimizer):
    ...
    def __init__(self, **kwargs):
        ...
        self.decomposer = kwargs.get("decomposer", False)
        self.prune_sources = kwargs.get("pruning", False)

        self.nlj_only = self.operators .get("bind_join_only", False)
        self.shj_only = self.operators.get("hash_join_only", False)

        assert not (self.nlj_only and self.shj_only), "NLJ only and SHJ only cannot be selected at the same time."
    ...
    def params_dct(self):
        params = {
            ...
            "bj_only" : self.nlj_only,
            "hj_only": self.shj_only
        }
    ...
    def create_plan(self, query):
        ...
        physical_plan = PhysicalPlan(self.sources, self.eddies, logical_plan, query, poly_operator=self.poly,
                                     ...
                                     brtpf_mappings=self.brtpf_mapping_cnt, **(self.operators))
    ...
    def get_physical_join_operator(self, left_plan, right_plan):
        ...
        # If root, we can be very sure to be correct. Therefore, dont force operator in the base/root case
        root = False #left_plan.is_triple_pattern and right_plan.is_triple_pattern

        # Just Place NLJ if planner configured this way
        if self.nlj_only and not root:
            return Xnjoin

        if self.shj_only and not root:
            return Fjoin
